...
class TestSeeder extends Seeder {
        ...
        foreach ($this->tables as $tableName => $columns) {
            Schema::connection('sqlite')->create($tableName, function ($table) use ($columns) {
                ...
                foreach ($columns as $column) {
                    if($column['inc']) {
                    ...
                    } else if (in_array($column['name'], ['created_at', 'updated_at'])) {
                        if(!$timestamps) {
                            $timestamps = true;
                        ...
                        switch ($column['type']) {
                            ...
                            case 'date':
                                ...
                                break;
                            case 'decimal':
                                $table->decimal($column['name'], 20, 2)
                                    ->default('0.00');
                                break;
                            case 'timestamp':
    ...
    private function cleanupType($type)
    {
        ...
        $type = preg_replace('/\([0-9]+,[0-9]+\)/', '', $type);
    ...
    {
        unlink(app_path() . '/database/test.sqlite');
        file_put_contents(app_path() . '/database/test.sqlite', '');
    }
